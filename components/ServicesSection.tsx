...
function getLocalizedServiceContent(service: Service, lang: string) {
  ...
  if (titleLower.includes('iklan') || titleLower.includes('advertising') || titleLower.includes('model') || titleLower.includes('talent')) {
    return {
      ...
      description: lang === 'ko'
        ? '인도네시아 브랜드 상품의 광고 모델 및 제품 모델로 한국 아이돌 가수 또는 한국 배우를 모델로 연결해 드릴것이며 상품 모델로 결정된 K\u2060-\u2060Pop 아이돌 포토카드를 제작하여 상품 판매시 증정할 수 있도록 하겠습니다. 또한 인도네시아에 진출하는 한국 브랜드 상품에 한국 또는 인도네시아 아티스트를 광고 모델 및 제품 모델로 연결해 드릴것이며 한국 상품의 인도네시아 마케팅에 크게 도움드리도록 하겠습니다.'
        : lang === 'en'
